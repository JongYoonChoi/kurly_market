...
int totalProduct=ProductDAO.getDAO().selectProductCount();
...
List<ProductDTO> productList = ProductDAO.getDAO().selectAllProductList(startRow,endRow);
...
<div class="page_aticle">
	...
	%>	
		...
		<% if(startPage>blockSize) { %>
			<a href="index.jsp?folder=product&category=Product_list&pageNum=1&search=">[&lt;&lt;]</a>
			<a href="index.jsp?folder=product&category=Product_list&pageNum=<%=startPage-blockSize%>">[&lt;]</a>
		<% } else { %>
			[&lt;&lt;][&lt;]
		<% } %>
		...
		<% for(int i=startPage;i<=endPage;i++) { %>
			<% if(pageNum!=i) { %>
				<a href="index.jsp?folder=product&category=Product_list&pageNum=<%=i%>">[<%=i %>]</a>
			<% } else { %>
				[<%=i %>]
			<% } %>
		<% } %>
		...
		<% if(endPage!=totalPage) { %>
			<a href="index.jsp?folder=product&category=Product_list&pageNum=<%=startPage+blockSize%>">[&gt;]</a>
			<a href="index.jsp?folder=product&category=Product_list&pageNum=<%=totalPage%>">[&gt;&gt;]</a>
		<% } else { %>
			[&gt;][&gt;&gt;]
		<% } %>
		</div>
