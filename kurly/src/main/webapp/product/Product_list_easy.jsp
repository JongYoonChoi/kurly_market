...
int totalProduct=ProductDAO.getDAO().selectEasyCount();
...
List<ProductDTO> productList = ProductDAO.getDAO().selectEasyProductList(startRow, endRow);
...
<div class="page_aticle">
	...
	%>	
		...
		<% if(startPage>blockSize) { %>
			<a href="index.jsp?&folder=product&category=Product_list_easy&pageNum=1&search=">[&lt;&lt;]</a>
			<a href="index.jsp?folder=product&category=Product_list_easy&pageNum=<%=startPage-blockSize%>">[&lt;]</a>
		<% } else { %>
			[&lt;&lt;][&lt;]
		<% } %>
		...
		<% for(int i=startPage;i<=endPage;i++) { %>
			<% if(pageNum!=i) { %>
				<a href="index.jsp?folder=product&category=Product_list_easy&pageNum=<%=i%>">[<%=i %>]</a>
			<% } else { %>
				[<%=i %>]
			<% } %>
		<% } %>
		...
		<% if(endPage!=totalPage) { %>
			<a href="index.jsp?folder=product&category=Product_list_easy&pageNum=<%=startPage+blockSize%>">[&gt;]</a>
			<a href="index.jsp?folder=product&category=Product_list_easy&pageNum=<%=totalPage%>">[&gt;&gt;]</a>
		<% } else { %>
			[&gt;][&gt;&gt;]
		<% } %>
		</div>
