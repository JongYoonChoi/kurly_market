...
int totalProduct=ProductDAO.getDAO().selectDishCount();
...
List<ProductDTO> dishList = ProductDAO.getDAO().selectDishProductList(startRow, endRow);
...
<div class="page_aticle">
	...
	<div id="goodsList" class="page_section section_goodslist">
		...
		<div class="list_goods">
			<div class="inner_listgoods">
				<ul class="list">
					<%for(ProductDTO product:dishList){ %>
					<li>
						<div class="item">
							<div class="thumb">
								<a class="img">
	...
	%>	
		...
		<% if(startPage>blockSize) { %>
			<a href="index.jsp?&folder=product&category=Product_list_dish&pageNum=1&search=">[&lt;&lt;]</a>
			<a href="index.jsp?&folder=product&category=Product_list_dish&pageNum=<%=startPage-blockSize%>">[&lt;]</a>
		<% } else { %>
			[&lt;&lt;][&lt;]
		<% } %>
		...
		<% for(int i=startPage;i<=endPage;i++) { %>
			<% if(pageNum!=i) { %>
				<a href="index.jsp?&folder=product&category=Product_list_dish&pageNum=<%=i%>">[<%=i %>]</a>
			<% } else { %>
				[<%=i %>]
			<% } %>
		<% } %>
		...
		<% if(endPage!=totalPage) { %>
			<a href="index.jsp?&folder=product&category=Product_list_dish&pageNum=<%=startPage+blockSize%>">[&gt;]</a>
			<a href="index.jsp?&folder=product&category=Product_list_dish&pageNum=<%=totalPage%>">[&gt;&gt;]</a>
		<% } else { %>
			[&gt;][&gt;&gt;]
		<% } %>
		</div>
